...
async function loadProductsFromAPI() {
  try {
    ...
    if (!prodErr && dbProducts && dbProducts.length > 0) {
      ...
      // Try cache before hardcoded fallback
      const cached = getCachedData('products');
      if (cached) {
        products = cached;
        console.warn('Using cached products (API unavailable)');
      } else {
    ...
    if (!preErr && dbPreowned && dbPreowned.length > 0) {
      ...
      const cached = getCachedData('preowned');
      if (cached) {
        preownedProducts = cached;
        console.warn('Using cached preowned products (API unavailable)');
      } else {
    ...
    // Cache products for offline resilience
    cacheProducts();
    ...
    // Try cache first
    const cached = getCachedData('products');
    const cachedPre = getCachedData('preowned');
    products = cached || getFallbackProducts();
    preownedProducts = cachedPre || getFallbackPreowned();
...
els.preownedLoadMore?.addEventListener('click', () => {
  state.preownedVisibleCount += state.preownedProductsPerLoad;
  renderPreowned();
});

// Product card clicks (both grids) — with improved event handling
document.addEventListener('click', (e) => {
  ...
  if (quickView) { e.preventDefault(); openModal(parseInt(quickView.dataset.id), quickView.dataset.condition); }
    ...
    if (product) {
      // Prevent duplicate additions of same product+size
      const existing = state.cart.findIndex(i => i.id === product.id && i.selectedSize === 'M' && i.condition === condition);
      if (existing !== -1) {
        state.cart[existing].quantity = (state.cart[existing].quantity || 1) + 1;
        showToast(`${product.name} quantity updated!`, 'success');
      } else {
        state.cart.push({ ...product, selectedSize: 'M', quantity: 1, condition });
...
function validateField(input) {
  ...
  if (input.type === 'tel' && value && value.replace(/[\s\-()]/g, '').length < 8) {
    showFieldError(input, 'Please enter a valid phone number');
    return false;
  }

  if (input.type === 'number' && input.min && parseInt(value) < parseInt(input.min)) {
...
// Attach blur validation to all required form fields
els.bulkForm?.querySelectorAll('input, select, textarea').forEach(field => {
...
els.bulkForm.addEventListener('submit', async (e) => {
  ...
  let isValid = true;
  els.bulkForm.querySelectorAll('input[required], select[required]').forEach(field => {
  ...
  const submitBtn = els.bulkForm.querySelector('.form-submit');
  if (submitBtn) {
  ...
  if (typeof InquiriesAPI !== 'undefined') {
    const { error } = await InquiriesAPI.create(sanitized);
    if (error) console.warn('Failed to save inquiry to DB:', error);
  }

  showToast(`Thank you, ${sanitized.full_name}! We've received your bulk order request and will contact you within 24 hours.`, 'success');
  els.bulkForm.reset();
...
// ============================================
function showSkeleton(gridElement, count = 8) {
  if (!gridElement) return;
  gridElement.innerHTML = Array(count).fill(null).map(() => `
    <div class="skeleton-card">
      ...
      <div class="skeleton-body">
        ...
        <div class="skeleton-line-sm" style="margin-top:12px"></div>
      </div>
    </div>
  `).join('');
}
...
// ============================================
// FORM DATA SANITIZATION (XSS prevention)
// ============================================
...
function loadStoreSettings() {
  try {
    ...
    // Update logo text safely
    const logoMain = document.querySelector('.logo-main');
    if (logoMain && saved.storeName) {
      logoMain.textContent = saved.storeName;
    }
    ...
    if (saved.storePhone) {
      document.querySelectorAll('.footer-contact-item .fa-phone').forEach((icon, i) => {
        ...
        if (phones[i]) {
          const parent = icon.closest('.footer-contact-item');
          if (parent) {
            const span = parent.querySelector('span:last-child');
            if (span) span.textContent = phones[i].trim();
          }
        }
      });
    }
    if (saved.storeEmail) {
      ...
      if (el) {
        const parent = el.closest('.footer-contact-item');
        if (parent) {
          const span = parent.querySelector('span:last-child');
          if (span) span.textContent = saved.storeEmail;
        }
      }
    }
    if (saved.storeAddress) {
      ...
      if (el) {
        ...
        if (parent) {
          const span = parent.querySelector('span:last-child');
          if (span) span.textContent = saved.storeAddress;
        }
      }
    }
...
// ============================================
// LOCALSTORAGE CACHING (Offline Resilience)
// ============================================
function cacheData(key, data) {
  try {
    localStorage.setItem(`corid_cache_${key}`, JSON.stringify({
      data,
      timestamp: Date.now()
    }));
...
function getCachedData(key, maxAge = 3600000) {
  try {
    const cached = JSON.parse(localStorage.getItem(`corid_cache_${key}`));
...
// Cache products after loading for offline resilience
function cacheProducts() {
  if (products.length > 0) cacheData('products', products);
  if (preownedProducts.length > 0) cacheData('preowned', preownedProducts);
}
...
showSkeleton(els.productGrid);
if (els.preownedGrid) showSkeleton(els.preownedGrid);  // Try loading cached data first for instant display
  const cachedProducts = getCachedData('products');
  const cachedPreowned = getCachedData('preowned');
  if (cachedProducts) {
    products = cachedProducts;
    preownedProducts = cachedPreowned || [];
    renderProducts();
    renderPreowned();
  }

  // Load fresh data from Supabase API (falls back to hardcoded data)
  loadProductsFromAPI();
